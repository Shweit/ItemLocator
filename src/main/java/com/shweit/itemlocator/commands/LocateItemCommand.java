...
public final class LocateItemCommand extends SubCommand {
    ...
    public void perform(final CommandSender commandSender, final Command command, final String label, final String[] args) {
        if (commandSender instanceof Player player) {
            if (args.length != 2) {
                commandSender.sendMessage(ChatColor.RED + Translator.getTranslation("usage_locate_command"));
            }

            Material material = Material.matchMaterial(args[1]);
            if (material == null) {
                commandSender.sendMessage(ChatColor.RED + Translator.getTranslation("material_not_found", Map.of("material", args[1])));
            }
